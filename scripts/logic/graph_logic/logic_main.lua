...
ddm_location = {}
ddm_location.__index = ddm_location
...
function ddm_location.new(name)
    local self = setmetatable({}, ddm_location)
    if name then
...
function ddm_location:connect_one_way(exit, rule)
...
function ddm_location:connect_two_ways(exit, rule)
...
function ddm_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function ddm_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function ddm_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function ddm_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function ddm_location:accessibility()
...
-- 
function ddm_location:discover(accessibility, keys)
...
entry_point = ddm_location.new("entry_point")
